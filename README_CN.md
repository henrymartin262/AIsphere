...
![Version](https://img.shields.io/badge/版本-3.0-22c55e?style=flat-square)
...
| **🔌 Agent 接入层** | MCP Server + REST 网关。AI Agent 可自动发现并接入 SealMind，无需阅读文档。 | 内置 + MCP 协议 |
...
SealMind 深度集成了 **0G 官方资源**——SDK、Agent Skills 和 Starter Kit——最大化协议原生功能，减少自定义代码。

### 📦 使用的官方 SDK

| SDK | 包名 | 在 SealMind 中的用途 |
|-----|------|---------------------|
| **0G TypeScript SDK** | `@0gfoundation/0g-ts-sdk ^0.3.3` | KV Storage 存储 Agent 记忆、Indexer 节点发现、Batcher 批量写入 |
| **0G Serving Broker** | `@0glabs/0g-serving-broker ^0.6.5` | TEE Provider 密封推理、动态 Provider 发现、费用自动结算 |

### 🤖 已集成的官方 0G Agent Skills

SealMind 使用 [0G Agent Skills](https://github.com/0gfoundation/0g-agent-skills) 官方仓库（克隆至 `.agent-skills/`）作为所有 0G 集成的权威参考。以下 Skills 已直接实现：
...
// metadataHash 写入链上 SealMindINFT.createAgent()
```
...
│   │   ├── contracts/
│   │   │   ├── SealMindINFT.sol      # Agent 身份 INFT（ERC-721）+ 灵魂签名
