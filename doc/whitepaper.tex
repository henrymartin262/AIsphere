...
{\normalsize
\textbf{Henry}\textsuperscript{1} \quad \textbf{Sirius Yao}\textsuperscript{1} \quad \textbf{Wystan}\textsuperscript{1}}
...
\subsection{TEE Trust Model}

AIsphere delegates inference to 0G Compute's Trusted Execution Environment (TeeML), which provides three hardware-enforced guarantees:

\begin{enumerate}[leftmargin=1.8em, itemsep=2pt, parsep=0pt]
  \item \textbf{Isolation.} The LLM runs inside a hardware enclave (Intel TDX). Neither the host OS, the hypervisor, nor the AIsphere server can observe or modify the computation.
  \item \textbf{Attestation.} The enclave produces a hardware-signed attestation $\sigma$ binding the model identity $h_\theta$, the input $h_x$, and the output $h_y$ into a single cryptographic statement: ``model $\theta$ produced output $y$ from input $x$ at time $t$ inside this specific enclave.''
  \item \textbf{Non-repudiation.} The attestation $\sigma$ is signed by the TEE's hardware root of trust. The platform cannot forge, modify, or replay it. Any third party holding $\sigma$ can verify it independently.
\end{enumerate}

This means: even if AIsphere is fully compromised, TEE-verified responses remain trustworthy because the proof originates from hardware, not software.
...
$$\pi = (h_\theta,\; h_x,\; h_y,\; \sigma,\; t,\; v,\; h_\pi)$$
where $h_\theta = \texttt{keccak256}(\theta)$ is the model hash, $h_x = \texttt{keccak256}(x)$ is the input hash, $h_y = \texttt{keccak256}(y)$ is the output hash, $\sigma$ is the TEE attestation signature (or $\bot$ if non-TEE fallback), $t$ is the Unix timestamp, $v \in \{0,1\}$ indicates whether the response carries a valid TEE attestation, and $h_\pi = \texttt{keccak256}(h_x \| h_y \| h_\theta \| t)$ is the composite proof hash anchored on-chain.
\end{defbox}

The critical distinction is $v$: when $v{=}1$, the proof is hardware-backed and the response is \emph{cryptographically verified}. When $v{=}0$, the system transparently indicates a non-TEE fallback tier---the user always knows the trust level.

\subsection{TEE Inference Flow}

\begin{algbox}[Algorithm 1: TEE-Verified Inference via 0G Compute]
\footnotesize
\textbf{Input:} prompt $x$, context $c$, model preference $\theta$ \\
\textbf{Output:} response $y$, proof $\pi$ \\[4pt]
1. \textbf{Provider discovery}: Query 0G Compute network for available TEE providers \\
2. \textbf{TEE selection}: Select provider $p$ with active TEE enclave \\
3. \textbf{Authentication}: Establish cryptographic session with provider's enclave \\
4. \textbf{Request signing}: Generate signed request headers binding $x$ to the session \\
5. \textbf{Enclave execution}: Model $\theta$ processes $x$ inside the TEE; produces $y$ and attestation $\sigma$ \\
6. \textbf{Fee settlement}: Settle inference cost on-chain via 0G Compute's payment protocol \\
7. \textbf{Proof construction}: Assemble $\pi = (h_\theta, h_x, h_y, \sigma, t, v{=}1, h_\pi)$ \\
8. \textbf{return} $(y, \pi)$
\end{algbox}

\noindent\textbf{Graceful degradation.} If TEE providers are temporarily unavailable, the system falls back to non-TEE LLM inference and ultimately to a deterministic local response. Each tier is transparently labeled in the proof tuple ($v{=}0$), so users can always distinguish hardware-verified responses from software-only ones. This ensures 100\% availability without silently degrading trust guarantees.
...
\textbf{0G Compute} & TEE inference via TeeML, provider discovery, fee settlement via \texttt{processResponse}, request signing; multi-layer degradation with transparent tier labeling \\[2pt]
\textbf{Agentic ID (ERC-7857)} & On-chain agent identity standard: ERC-721 INFT with soul signature, passport certification, trust score, 5-tier level system, ownership transfer with memory migration \\[2pt]
\textbf{0G Memory} & Persistent agent memory across sessions via 0G Storage KV; dual-layer architecture (in-memory hot cache + 0G KV); hydration on startup; graceful degradation to cache-only mode \\[2pt]
\textbf{TEE / Private Sandbox} & Sealed Mind runs all primary inference inside hardware enclave (Intel TDX via TeeML); hardware-signed attestation per response; enclave isolation ensures neither host nor platform can observe computation \\[2pt]
...
To our knowledge, AIsphere is the first project to deeply integrate \emph{all six} 0G Network components plus seven official Agent Skills in a single coherent application. Most projects at the time of writing use only one or two components (typically Chain alone).
