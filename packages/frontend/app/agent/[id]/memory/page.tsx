  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Message list — when timestamps equal, user always before assistant */}
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {[...(session?.messages ?? [])]
            .sort((a, b) => {
              const tsDiff = a.timestamp - b.timestamp;
              if (tsDiff !== 0) return tsDiff;
              // Same timestamp: user first, assistant second
...
export default function AgentMemoryPage() {
  ...
  // sessions used by all tab
  const sessions_all = useLocalChatSessions(agentId);

  // Render a list of MemoryItems (shared by all-tab and specific tabs)
  function MemoryItemList({ items }: { items: MemoryItem[] }) {
    return (
      <div className="space-y-3">
        {items.map((memory) => {
          ...
          return (
            <div key={memory.id} className="card group relative p-5 hover:shadow-md">
              ...
              <div className="flex items-start gap-3">
                <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border text-base ${colorClass}`}>{icon}</div>
                <div className="flex-1 min-w-0">
                  <span className={`inline-block rounded-lg border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${colorClass}`}>{memType}</span>
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-3">
                    ...
                    {memory.tags && memory.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {memory.tags.map((tag) => (
                          <span key={tag} className="rounded-md border border-gray-100 bg-gray-50 px-1.5 py-0.5 text-[10px] text-gray-400 dark:border-white/[0.06] dark:bg-white/[0.03] dark:text-slate-500">#{tag}</span>
                        ))}
                      </div>
                    )}
                  </div>
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-8">
      ...
      {/* Conversation tab: local chat sessions (session cards) */}
      {activeTab === "conversation" && <ConversationTab agentId={agentId} />}

      {/* All tab: sessions + all non-conversation memories */}
      {activeTab === "all" && (
        <>
          {/* Sessions section */}
          {sessions_all.length > 0 && (
            <div className="mb-4">
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500">
                💬 {sessions_all.length} {sessions_all.length === 1 ? "对话" : "条对话"}
              </p>
          ...
          {!isLoading && filtered.length > 0 && (
            <>
              ...
              <MemoryItemList items={filtered} />
            </>
          )}
          {!isLoading && filtered.length === 0 && sessions_all.length === 0 && (
            <div className="flex flex-col items-center gap-3 py-20 text-center">
              <span className="text-5xl">🧠</span>
      ...
      {activeTab !== "conversation" && activeTab !== "all" && activeTab !== "skill" && (
        <>
          ...
          {!isLoading && filtered.length > 0 && <MemoryItemList items={filtered} />}
        </>
      )}
